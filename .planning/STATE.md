...
- /api/healthz → 500 OK (Zod-validated shape, correlationId UUID, ISO timestamp — env check fails sem SUPABASE_SERVICE_ROLE_KEY na Vercel, esperado)
- PLAYWRIGHT_BASE_URL setado no GH Secrets → e2e-gate vai liberar e2e job na próxima CI run
