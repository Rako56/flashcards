...
| 1.3-T2 | 01-03 | 3 | FOUND-05 | T-1.3-01, T-1.3-03, T-1.3-04 | vitest.config.ts with per-path coverage thresholds + MSW server with onUnhandledRequest error | auto | `pnpm typecheck && grep -q "lib/srs/" vitest.config.ts && grep -q "onUnhandledRequest: 'error'" tests/setup.ts` | flashcards/vitest.config.ts | ✅ green (Plan 1.3 a6a9245) |
| 1.3-T3 | 01-03 | 3 | FOUND-05 | T-1.3-02 | unit tests for cn() + env() pass | auto+tdd | `pnpm test tests/unit/utils.test.ts tests/unit/env.test.ts` | flashcards/tests/unit/utils.test.ts | ✅ green (Plan 1.3 a6a9245) |
| 1.3-T4 | 01-03 | 3 | FOUND-05 | T-1.3-01 | Coverage thresholds engaged via 4 placeholder modules | auto+tdd | `pnpm test --coverage` | flashcards/lib/srs/index.ts | ✅ green (Plan 1.3 a6a9245) |
| 1.3-T5 | 01-03 | 3 | FOUND-05 | n/a | Plan 1.3 committed | auto | `git log --oneline -1 | grep -q "Plan 1.3\\|Vitest\\|coverage"` | n/a | ✅ green (a6a9245) |
| 1.4-T1 | 01-04 | 4 | FOUND-06 | T-1.4-SC | Playwright + Chromium installed | auto | `pnpm list @playwright/test && pnpm exec playwright --version` | flashcards/package.json | ✅ green (Plan 1.4 fa09610) |
| 1.4-T2 | 01-04 | 4 | FOUND-06 | n/a | playwright.config.ts compiles + Chromium ready | auto | `pnpm typecheck && pnpm exec playwright test --list` | flashcards/playwright.config.ts | ✅ green (config parses; --list lists tests when present, see 01-04-SUMMARY) |
| 1.4-T3 | 01-04 | 4 | FOUND-05 | T-1.4-01 | Per-path coverage gate-break probe fires | auto | inline probe script (create uncovered-temp.ts → coverage fails → revert → coverage passes) | n/a | ✅ green (exit 1 with `Coverage for lines (2.85%) does not meet "lib/srs/**" threshold (90%)`; revert → exit 0; Plan 1.4 fa09610) |
| 1.4-T4 | 01-04 | 4 | FOUND-06 | T-1.4-02 | Plan 1.4 committed, no probe file leaked | auto | `git log --oneline -1 | grep -q "Plan 1.4\\|Playwright\\|gate-break"` | n/a | ✅ green (fa09610; `git ls-files lib/srs/` clean) |
...
| Coverage 90% core | Add uncovered code in `src/lib/srs/` | CI test step fails red with coverage below 90% in that path | ✅ Plan 1.4 (fa09610) — observed `ERROR: Coverage for lines (2.85%) does not meet "lib/srs/**" threshold (90%)` |
